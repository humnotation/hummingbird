...
MusicXMLParser.measureAttributes = function(renderer) {
...
_.extend(MusicXMLParser.measureAttributes.prototype, {
    ...
    decorateStaves: function(drawClefs, drawKeySig, drawTimeSig) {
        ...
        if (drawClefs === true) {
            ...
            for (clefNumber in this.clefs) {
                ...
                if (clefType) {
                    this.renderer.renderClef(clefNumber, clefType, clef.line);
                } else {
        ...
        if (drawTimeSig === true) {
            this.renderer.renderTimeSignature(this.beatsPerMeasure, this.beatsType);
        }
        
        // the boolean depends on whether it is the right spot on the page to print the signature
        // from keyFifths and keyMode
        if (drawKeySig === true) {
            var key = this.keySymbol[this.keyFifths + "," + this.keyMode];
            if (key) {
                this.renderer.renderKeySignature(key);
            } else {
                throw new Error("unsupported key type: fifths=" + this.keyFifths + ", mode=" + this.keyMode);
            }
        }    
    }
