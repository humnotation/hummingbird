...
MusicXMLParser.notes = function(options)
{
...
_.extend(MusicXMLParser.notes.prototype, {
    ...
    */
    ...
    addNotes: function ($notes) {
        for (var k in $notes) {
            if ($notes[k].nodeName === "note") {
                this.assimilateNote(new MusicXMLParser.note($notes[k], this.ties));
    ...
    isNoteFitChord: function(note, voice) {
        ...
        if (ret === false) {
                throw new Error("note: These incompatible notes are trying to fit in the same chord in this voice: " ,note1, note, voice);
        }
        return ret;
    },
    ...
    render: function () {
        ...
        for (vindex in this.voices) {
            ...
            for (cindex in voice.chords) {
                ...
                for (nindex in chord) {
                    ...
                    if (! note.isRest) {
                        ...
                        if (note.tie.isStop) {
                            this.ties.matchTies(note, nindex, vexNote);
                        }
                        // If the note is the start of a tie, remember it so it can be matched with its end
                        if (note.tie.isStart) {
                            this.ties.rememberTie(note, nindex, vexNote);
                        }
                    }
                }
            ...
            this.renderer.renderNotes(voice.staff, noteArray);
            
            // Draw all the matched ties
            this.ties.renderTieMatches();
        }
    },
    backup: function() {
        this.render();
...
MusicXMLParser.note = function(note, ties)
{
...
_.extend(MusicXMLParser.note.prototype, {
